...
class KNNCalculator:
  ...
  def GetResponse(self,neighbors):
      ...
      for item in neighbors:
          res += item[-1]
      return res/len(neighbors)

  def GetAccuracy(self,prediction):
      stdAcc = 0
      for i in range(len(prediction)):
          #diff = math.pow(self.testSet[i][-1] - prediction[i],2)
          #stdAcc += diff
          stdAcc += abs(self.testSet[i][-1] - prediction[i]) / self.testSet[i][-1]
      return (1 - stdAcc/len(prediction))*100


  @staticmethod
  def MinMaxNormalize(data):
      for i in range(len(data[0])-1):
          minData = min(data,key=operator.itemgetter(i))[i]
          maxData = max(data,key=operator.itemgetter(i))[i]
          for j in range(len(data)):
              data[j][i] = (data[j][i] - minData)/(maxData-minData)
      return data
  ...
  def Compute(self,splitMethod = None):
      if self.id == 0 and self.thread != 1:
          ...
          for i in range(self.thread):
              ...
              currentKNN = KNNCalculator(self.data,self.comparator,self.kLimit,1,self.skipFirst,self.paralel,i,self.sharedList)
...
if __name__ == '__main__':
    ...
    KNNInstance = KNNCalculator(fileName,None,skipFirst=False)
    dataset = KNNCalculator.MinMaxNormalize(KNNInstance.data)
   
    distanceMethod = [euclideanDistance,manhattanDistance,minkowskiDistance,cosineSimilarity]
    for method in distanceMethod:
        for kVal in range(kLimit):
            print("Computing Using " + method.__name__ + " K = " + str(kVal+1),end=" ")
            KNNNode = KNNCalculator(dataset,comparator=method,kLimit=kVal+1,thread=threadCount,id=0,paralel=True)
            res = KNNNode.Compute(splitMethod=KNNCalculator.StratifiedSplit)
            #for data in res[1]:
                #print("Accuracy : %.2f" % data)
            print("Accuracy Using Mean Absolute Percentage Error (MAPE) Average For %s K = %d: %.2f" % (method.__name__,kVal+1,res[0]))
